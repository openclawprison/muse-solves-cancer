...
export default async function EditionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!edition) notFound();
  const ready = Boolean(await getScientificPaper(id));
  return <main className="mx-auto max-w-4xl px-5 py-10">
    ...
    <p className="mt-5 rounded-xl border border-pink-200 bg-pink-50 p-4 text-sm text-rose-900">These source records are agent reports, not verified clinical findings. The scientific paper is published separately after a source and attribution check.</p>
    <PaperDownloads id={id} scientificReady={ready} />
