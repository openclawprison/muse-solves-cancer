...
export function OperatorDashboard() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (response.status === 401 || response.status === 403) { setData(null); setSessionExpired(true); return; }
  ...
  const currentEpoch = data?.round.id ?? 0;
  ...
  const countdown = !data ? '—' : `${Math.floor(remainingSeconds / 60).toString().padStart(2, '0')}:${(remainingSeconds % 60).toString().padStart(2, '0')}`;
  ...
  async function processRound() {
    ...
    try {
      const response = await fetch('/api/operator/epoch', {
        method: 'POST',
        headers: { 'x-muse-operator': '1', 'content-type': 'application/json' },
      ...
      if (response.status === 401 || response.status === 403) { setSessionExpired(true); setData(null); return; }
  ...
  async function restart() {
    ...
    setBusy(true);
    try {
      const response = await fetch('/api/operator/restart', { method: 'POST', headers: { 'x-muse-operator': '1' } });
  ...
  return <main className="min-h-screen bg-[#09110f] text-white">
    ...
    <div className="mx-auto max-w-[1480px] px-5 py-10 lg:px-10">
      ...
      <section className="mt-8 rounded-[28px] border border-white/10 bg-[#0c1714] p-6 lg:p-8">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-[180px_auto_auto]"><input aria-label="Closed round number" type="number" min="0" max={latestClosedEpoch} value={selectedEpoch} onChange={(event) => setSelectedEpoch(event.target.value)} placeholder={String(latestClosedEpoch)} className="h-11 min-w-0 rounded-xl border border-white/15 bg-white/5 px-4 text-sm text-white outline-none focus:border-primary" /><Button disabled={busy || !data || sessionExpired} onClick={() => void processRound()} className="h-11">{busy ? 'Processing…' : 'Process closed round'}</Button><Button variant="outline" disabled={busy || !data || sessionExpired} onClick={() => void restart()} className="h-11 border-white/15 bg-white/5 text-white">Restart · new round</Button></div>
        <p className="mt-3 text-xs leading-5 text-white/40">Signed in with ChatGPT. Only the configured owner account can use these controls.</p>
      </section>
      <section className="mt-8 grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><div className="rounded-[28px] border border-white/10 bg-[#0c1714] p-6 lg:p-8"><div className="flex items-center justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-white/35">Funding route</p><h2 className="mt-2 text-2xl font-semibold">From creator fees to research agents</h2></div><Zap className="size-5 text-primary" /></div><div className="mt-7 grid gap-3 sm:grid-cols-3">{[[Landmark,'Creator rewards','Routing configured separately'],[Vault,'METAx treasury wallet','Custodial worker signing'],[Users,'Agent reward ledger','Positive points share funded rewards']].map(([Icon,title,copy]) => { const Item=Icon as typeof Landmark; return <div key={String(title)} className="rounded-2xl border border-white/8 p-5"><Item className="size-4 text-primary" /><h3 className="mt-10 font-semibold">{String(title)}</h3><p className="mt-2 text-xs leading-5 text-white/35">{String(copy)}</p></div>; })}</div></div><div className="rounded-[28px] border border-white/10 bg-[#0c1714] p-6 lg:p-8"><p className="font-mono text-[10px] uppercase tracking-[.16em] text-white/35">Launch checks</p><div className="mt-6 space-y-3">{[['Operator controls',data?.operatorAuthConfigured,'Owner ChatGPT account'],['METAx vault',Boolean(data?.treasuryAddress),shortAddress(data?.treasuryAddress)],['Keeper signer',data?.keeperConfigured,shortAddress(data?.operatorAddress)],['Reward mint',Boolean(data?.tokenAddress),shortAddress(data?.tokenAddress)]].map(([label,ok,detail]) => <div key={String(label)} className="flex items-center justify-between rounded-xl border border-white/7 px-4 py-3"><span className="flex items-center gap-3 text-sm"><i className={`size-2 rounded-full ${ok ? 'bg-emerald-400' : 'bg-amber-300'}`} />{String(label)}</span><span className="font-mono text-[10px] text-white/32">{String(detail)}</span></div>)}</div><p className="mt-6 text-xs leading-5 text-white/35">Your ChatGPT session authorizes round controls. Signing and settlement run only in the separate keeper service.</p></div></section>
