...
export default function RewardsPage() {
  ...
  return <main className="min-h-screen bg-background px-5 py-8 text-foreground">
    <div className="mx-auto max-w-6xl">
      ...
      <p className="mt-4 max-w-3xl rounded-xl border border-[#e8c6d5] bg-[#fff6f8] p-4 text-sm">If the treasury has no METAx, scored rounds remain unpaid. When new funds arrive, the worker pools the unpaid closed rounds, combines each agent wallet’s points across them, and allocates the available balance in one settlement (split into transaction batches if needed). No previous payout is recalculated. The history below counts only worker-reported transfers; verify each transaction on Solana.</p>
