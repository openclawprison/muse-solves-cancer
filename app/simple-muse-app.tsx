...
import { DailyArticleCard } from '@/components/daily-article-card';
...
export function SimpleMuseApp({ initialDailyArticle }: { initialDailyArticle: DailyArticle | null }) {
  ...
  return <main className="min-h-screen bg-[#faf7f2] text-[#30272b]">
    ...
    <div className="mx-auto max-w-6xl px-5 sm:px-8">
      ...
      <section id={'panel-'+tab.toLowerCase()} role="tabpanel" aria-labelledby={'tab-'+tab.toLowerCase()} tabIndex={0} className="min-h-[420px] py-8 outline-none">
        {tab === 'Overview' && <>
          <DailyArticleCard initialArticle={initialDailyArticle} />
          <RoundProgress />
        ...
        {tab === 'Research' && <><DailyArticleCard initialArticle={initialDailyArticle} /><h2 className="font-serif text-3xl">From sources to findings.</h2><p className="mt-3 mb-6 text-sm text-[#72676a]">Choose where you want to explore. Sources are not findings until reviewed.</p><div className="grid gap-4 sm:grid-cols-2"><Resource href="/research" title="Papers & clinical trials">Search the source catalogue and open the original evidence.</Resource><Resource href="/science" title="Claims & verification">Trace claims to sources, independent checks, and challenges.</Resource><Resource href="/papers" title="Living Paper">Read the dated research findings, source notes, limitations, and next steps online.</Resource></div></>}
