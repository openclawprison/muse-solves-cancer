...
export function validateScores(payload, rows) {
  let scores;
  try { const text=(payload.output??[]).flatMap(x=>x.content??[]).filter(x=>x.type==='output_text').map(x=>x.text??'').join('');scores=JSON.parse(text).scores; } catch { throw Error('Scoring response was not valid JSON.'); }
...
function requestBody(rows,epochId,model){
  ...
  return {model,store:false,background:true,reasoning:{effort:'low'},tools:[{type:'web_search'}],
    ...
    input:JSON.stringify({epochId,submissions:rows}),text:{format:{type:'json_schema',name:'muse_batch_scores',strict:true,schema:{type:'object',properties:{scores:{type:'array',minItems:rows.length,maxItems:rows.length,items:{type:'object',properties,required:Object.keys(properties),additionalProperties:false}}},required:['scores'],additionalProperties:false}}}};
}
...
export async function scoreInBatches({db,rows,epochId,model,apiKey,fetcher=fetch,now=Date.now()}){
  ...
  if(jobs.some(j=>j.failure_count>=3&&!j.scores_json))throw Error('A scoring batch needs operator review after three invalid/failed responses. Completed batches are preserved.');
  ...
  const attempts=await Promise.allSettled(work.map(async job=>{
    ...
    try {
      ...
      const updates=[db.prepare('UPDATE scoring_batches SET response_id=NULL,failure_count=failure_count+1,next_attempt_at=?,last_error=? WHERE id=?').bind(now+15000,reason,job.id)];
