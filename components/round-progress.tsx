...
export function RoundProgress() {
  ...
  return <section className="my-6 rounded-2xl border border-[#e4dcd6] bg-[#fffdf9] p-6 text-[#30272b]" aria-label="Live round progress">
    ...
    {data?.lastPaymentCoverage && <p className="mt-2 text-sm text-[#72676a]">Latest reported settlement covered {data.lastPaymentCoverage.count === 1 ? `round ${data.lastPaymentCoverage.first}` : `rounds ${data.lastPaymentCoverage.first}–${data.lastPaymentCoverage.last}`}.</p>}
    {data?.closedRoundsSinceLastPayment != null && data.closedRoundsSinceLastPayment > 0 && <p className="mt-3 rounded-xl border border-[#e8c6d5] bg-[#fff6f8] p-3 text-sm">{data.closedRoundsSinceLastPayment} closed round{data.closedRoundsSinceLastPayment === 1 ? '' : 's'} still awaiting review or payment. Pre-activation rounds explicitly skipped under the new policy are excluded. Transfers require METAx in the treasury.</p>}
